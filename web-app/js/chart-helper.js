...
// Old style: Table
SurveyChartRenderer.prototype.forScaleTableStyle = function(scaleLabels, itemValuesRows, canvasObj, title) {
  ...
  for (var i = 0; i < numOfRows; i++) {
    ...
    for (var j = 0; j < length; j++) {
      ...
      td.appendChild(document.createTextNode(' (' + Math.round(percentageJ) + '%)'));
...
SurveyChartRenderer.prototype.forScale = function(scaleLabels, itemValuesRows, canvasObj, title) {
  ...
  var data = [];
  for (var i = 0; i < itemValuesRows[0].values.length; i++) {
    // For each column

    var oneColorBarData = [];
    for (var j = 0; j < itemValuesRows.length; j++) {
      // For each rows

      var value = itemValuesRows[j].values[i];
      var label = itemValuesRows[j].label;
      oneColorBarData[j] = [value , label];
    }

    data[i] = oneColorBarData;
  }

  /*
  data = [
    [[2,'Doraemon'], [4,'Power Rangers'], [6,'Samurai X'], [3,'Mojacko']],
    [[5,'Doraemon'], [1,'Power Rangers'], [3,'Samurai X'], [4,'Mojacko']],
    [[4,'Doraemon'], [7,'Power Rangers'], [1,'Samurai X'], [2,'Mojacko']]
  ]
  */

  var f = 0;
  var flatLabels = []; // in flat array
  ...
  for (var i = 0; i < itemValuesRows[0].values.length; i++) {
    ...
    for (var j = 0; j < itemValuesRows.length; j++) {
      // For each rows
      var percentages = this.calcPercentage(itemValuesRows[j].values);

      var value = itemValuesRows[j].values[i] + ' (' + Math.round(percentages[i]) + '%)'; // '21 (5%)';
      var label = itemValuesRows[j].label;
      oneColorBarData[j] = value;

      flatLabels[f++] = value;
    }

    labels[i] = oneColorBarData;
  }

  f = 0;
  $.jqplot.LabelFormatter = function(format, val) {
    // return val + '(' + (val / 87 * 100).toFixed(2) + '%)';
    return flatLabels[f++];
  }

  var series = [];
  for (var i = 0; i < scaleLabels.length; i++) {
    series[i] = {label: scaleLabels[i]};
  }

  plot2b = canvasObj.jqplot(data, {
            seriesDefaults: {
                renderer:$.jqplot.BarRenderer,
                pointLabels: {
                    show: true,
...
SurveyChartRenderer.prototype.adjustChoiceCanvas = function(target, labels) {
  ...
  target.css('height', labels.length * 55);
}
